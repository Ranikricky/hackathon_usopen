<template>
  <div class="process-page">
    <!-- 顶部导航栏 -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">HORIZON XL</div>
...
const getApiErrorMessage = (err, fallback = '请求失败') => {
...
const handleNewProject = async () => {
  ...
  try {
    ...
    ontologyProgress.value = { message: '正在准备输入并分析内容...' }
    ...
    error.value = '项目初始化失败: ' + getApiErrorMessage(err, '未知错误')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    error.value = '加载项目失败: ' + getApiErrorMessage(err, '未知错误')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    error.value = '启动图谱构建失败: ' + getApiErrorMessage(err, '未知错误')
    buildProgress.value = null
  }
}
