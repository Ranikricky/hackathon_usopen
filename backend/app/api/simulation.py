...
def generate_structured_agents():
    ...
    try:
        ...
        if not domain_plan:
            ...
            domain_plan["generation_seed"] = f"{project_id or 'prompt'}:{uuid.uuid4().hex[:12]}"
        elif data.get("project_id"):
            project = ProjectManager.get_project(data.get("project_id"))
            if project and project.generation_seed:
                domain_plan["generation_seed"] = project.generation_seed
        else:
