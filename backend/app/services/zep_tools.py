...
class InsightForgeResult:
    ...
    def to_text(self) -> str:
        ...
        if self.semantic_facts:
            text_parts.append(f"\n### Key Facts (cite these source facts in English)")
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        if self.active_facts:
            text_parts.append(f"\n### Active Facts (source simulation output)")
...
class ZepToolsService:
    ...
    def __init__(self, api_key: Optional[str] = None, llm_client: Optional[LLMClient] = None):
        ...
        self.client = None
        if self.api_key and not self._is_placeholder_secret(self.api_key):
    ...
    def search_graph(
        ...
        # 本地图谱或无可用Zep客户端时，直接走本地关键词检索
        if self._is_local_graph(graph_id) or not self.client:
        ...
        if self._is_local_graph(graph_id):
            ...
            for edge in edges:
                edge_info = EdgeInfo(
                    ...
                    name=str(edge.get("name", "") or edge.get("fact_type", "")),
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        ...
        try:
            if node_uuid.startswith("local_") and "_node_" in node_uuid:
                graph_id = node_uuid.split("_node_", 1)[0]
                graph_data = self._get_local_graph_data(graph_id)
                for node in graph_data.get("nodes", []) or []:
                    if str(node.get("uuid", "")) == node_uuid:
                        return NodeInfo(
                            uuid=str(node.get("uuid", "")),
                            name=str(node.get("name", "")),
                            labels=node.get("labels", []) or [],
    ...
    def _generate_sub_queries(
        ...
        """
        system_prompt = """You are an expert question analyst. Decompose a complex report question into specific English sub-questions that can be independently searched in the simulated world.

Requirements:
1. Each sub-question must be concrete enough to find related agent behavior, facts, or events.
2. Cover different dimensions: who, what, why, how, when, where, and numerical evidence when relevant.
3. Keep every sub-question related to the simulation scenario.
...
Break the following question into {max_queries} English sub-questions:
{query}
...
Return JSON only."""
    ...
    def interview_agents(
        ...
        # Add an English prefix to constrain agent response format.
        INTERVIEW_PROMPT_PREFIX = (
            "You are being interviewed. Use your persona, memories, and actions to answer the following questions in English plain text.\n"
            "Response requirements:\n"
            "1. Answer directly in natural language; do not call tools.\n"
            "2. Do not return JSON or tool-call syntax.\n"
            "3. Do not use Markdown headings.\n"
            "4. Answer question by question. Start each answer with 'Question X:' where X is the question number.\n"
            "5. Separate answers with blank lines.\n"
            "6. Each answer must contain substantive detail, at least 2-3 sentences.\n\n"
        )
        ...
        try:
            ...
            if not api_result.get("success", False):
                ...
                result.summary = f"Interview API call failed: {error_msg}. Check that the OASIS simulation environment is running."
                return result
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Normalize response text before extracting short English quotes.
                clean_text = re.sub(r'#{1,6}\s+', '', combined_responses)
                clean_text = re.sub(r'\{[^}]*tool_name[^}]*\}', '', clean_text)
                clean_text = re.sub(r'[*_`|>~\-]{2,}', '', clean_text)
                clean_text = re.sub(r'(Question|Q)\s*\d+\s*[:：]\s*', '', clean_text, flags=re.IGNORECASE)
                clean_text = re.sub(r'【[^】]+】', '', clean_text)

                # Strategy 1: extract complete, substantive English sentences.
                sentences = re.split(r'(?<=[.!?])\s+', clean_text)
                meaningful = [
                    s.strip() for s in sentences
                    if 20 <= len(s.strip()) <= 150
                    and not re.match(r'^[\s\W，,；;：:、]+', s.strip())
                    and not s.strip().startswith(('{', 'Question', 'Q'))
                ]
                meaningful.sort(key=len, reverse=True)
                key_quotes = [s if s.endswith(('.', '!', '?')) else f"{s}." for s in meaningful[:3]]

                # Strategy 2: extract quoted English text if sentence splitting found nothing.
                if not key_quotes:
            ...
            result.summary = f"Interview failed: {str(e)}. The simulation environment may be closed; make sure OASIS is running."
            return result
        except Exception as e:
            logger.error(t("console.interviewApiCallException", error=e))
            import traceback
            logger.error(traceback.format_exc())
            result.summary = f"An interview error occurred: {str(e)}"
            return result
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are an expert interview planner. Select the most relevant simulated agents for the interview request. Write English only.

Selection criteria:
1. The agent identity/profession is relevant to the interview topic.
2. The agent may hold a distinct or valuable perspective.
3. Select diverse viewpoints where possible.
4. Prefer agents directly connected to the event or forecast.

Return JSON only:
{
    "selected_indices": [0, 1],
    "reasoning": "English selection rationale"
}"""
...
Select at most {max_agents} agents and explain the choice in English."""
    ...
    def _generate_interview_questions(
        ...
        system_prompt = """You are an expert journalist/interviewer. Generate 3-5 deep English interview questions.

Requirements:
1. Use open-ended questions that encourage detailed answers.
2. Questions should allow different roles to answer differently.
3. Cover facts, opinions, incentives, feelings, and forecast reasoning where relevant.
4. Sound natural, like a real interview.
5. Keep each question concise.
6. Ask directly, with no background preamble.
...
Generate 3-5 English interview questions."""
        ...
        try:
            ...
            return [
                ...
                "What evidence, risks, or tradeoffs matter most to your view?"
            ]
    ...
    def _generate_interview_summary(
        ...
        system_prompt = """You are an expert news editor. Summarize multiple interview answers in English.
...
- You may use **bold** for key labels."""
...
Generate an English interview summary."""
